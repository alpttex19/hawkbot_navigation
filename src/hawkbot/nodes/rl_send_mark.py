...
# 修改后的回调函数，适用于RL导航
def rl_nav_status_callback(msg):
    """RL导航状态回调 - 当到达目标时触发"""
    global try_again, index, count
...
def send_mark():
    ...
    rospy.init_node('rl_path_point_demo')  # 初始化节点
    ...
    # 订阅RL导航状态（替换原来的move_base状态）
    rl_status_sub = rospy.Subscriber('/rl_nav/goal_status', PoseStamped, rl_nav_status_callback)
    ...
    rospy.sleep(1.)
    sendflagPublisher.publish(send_flag)
    rospy.loginfo('send_flag=%d', send_flag.data)
    print("###############################")
    print("RL Navigation Send Mark Node Ready")
    print("Use 'c' to clear all waypoints")
